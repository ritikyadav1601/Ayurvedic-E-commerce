...
const Header = () => {
  ...
  let totalQty = 0;
  try {
    // Use context if provider mounted; fallback to 0 during SSR mismatch
    // We guard to avoid throwing if Header ever renders outside provider
    // TypeScript can't narrow conditional hook usage, so we keep simple try/catch
    // eslint-disable-next-line react-hooks/rules-of-hooks
    totalQty = useCart().totalQuantity;
  } catch {}
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="hidden md:flex gap-6 items-center">
          <Link href='/cart' className='relative'>
            ...
            {totalQty > 0 && (
              <span className='absolute -top-2 -right-2 bg-[#C99A3D] text-white text-xs rounded-full px-1.5'>{totalQty}</span>
            )}
          </Link>
          <Link href='/auth/login' className='font-inter font-semibold hover:text-[#C99A3D] transition-colors'>
            Login / SignUp
          </Link>
        </div>
      ...
      {menuOpen && (
        <div className="mt-4 flex flex-col gap-4 md:hidden font-inter font-semibold text-[#5C6B57]">
          ...
          <div className="flex gap-6 items-center mt-2">
            <Link href='/cart' onClick={() => setMenuOpen(false)} className='relative'>
